# 🎧 귀띔(EarBrief) 개발 기획서 v2.2

> **문서 버전**: v2.2 (초저지연 듀얼 에이전트 아키텍처 + 최첨단 기술 통합판)
> **작성일**: 2026-03-13
> **기반 문서**: 귀띔_EarBrief_서비스기획서_v1.0 / 개발기획서_v2.1
> **목적**: v2.1 대비 듀얼 에이전트 파이프라인, 초저지연 스트리밍 TTS, 시간인식 지식그래프 등 차세대 기술을 통합한 상세 개발 명세
> **v2.2 핵심 변경**: Cartesia Sonic TTS(TTFA 40ms) / NVIDIA Parakeet TDT(온디바이스 STT) / Zep Graphiti(시간인식 지식그래프) / LlamaPIE 듀얼 모델 개입 / Speculative Decoding / Eager End-of-Turn / 4-Phase 재설계
...
10. [Phase별 상세 구현 계획 (4-Phase)](#10-phase별-상세-구현-계획) ★ RESTRUCTURED
...
16. [접근성(Accessibility) 전략](#16-접근성accessibility-전략)
17. [국제화(i18n) 아키텍처](#17-국제화i18n-아키텍처)
18. [앱 크기 최적화 전략](#18-앱-크기-최적화-전략)
19. [위젯 & Quick Settings Tile](#19-위젯--quick-settings-tile)
20. [API Rate Limiting & 쿼터 관리](#20-api-rate-limiting--쿼터-관리)
21. [데이터 마이그레이션 전략](#21-데이터-마이그레이션-전략)
22. [에러 처리 통합 전략](#22-에러-처리-통합-전략)
23. [듀얼 에이전트 파이프라인 (Slow Thinker / Fast Talker)](#23-듀얼-에이전트-파이프라인) ★★ v2.2 NEW
24. [초저지연 스트리밍 TTS 파이프라이닝](#24-초저지연-스트리밍-tts-파이프라이닝) ★★ v2.2 NEW
25. [시간인식 지식그래프 (Temporal Knowledge Graph)](#25-시간인식-지식그래프) ★★ v2.2 NEW
26. [LLM 추론 최적화 (Speculative Decoding & Eager EOT)](#26-llm-추론-최적화) ★★ v2.2 NEW
...
## 23. 듀얼 에이전트 파이프라인 (Slow Thinker / Fast Talker)

> **v2.2 신규 추가 섹션** — LlamaPIE(ACL 2025) 듀얼 모델 파이프라인 + VoiceAgentRAG 융합
...
│  Silero VAD (<1ms) → Flush → STT (Deepgram Nova-3)   │
└──────────┬───────────────────────────────┬────────────┘
           │ 실시간 전사 스트림                │
     ┌─────▼─────────────┐         ┌───────▼──────────┐
     ...
     │                   │         │                  │
     │ • 대화 맥락 분석    │  Cache  │ • 캐시 즉시 조회   │
     │ • 후속 질문 예측    │────────▶│   (< 0.35ms)     │
     │ • RAG 프리페치     │         │ • 1~3 단어 힌트    │
     │ • 지식그래프 업데이트│         │ • TTS 즉시 전달   │
     └───────────────────┘         └────────┬─────────┘
                                            │
                                   ┌────────▼─────────┐
                                   │  Cartesia Sonic   │
...
### 23.2 Slow Thinker (백그라운드 에이전트)

**역할**: 고객 발화 중 실시간으로 문맥을 파악하여 '직원이 다음에 필요할 정보'를 **미리** 검색해 캐시에 적재

```kotlin
class SlowThinkerAgent(
    private val contextAnalyzer: ContextAnalyzer,
    private val ragEngine: VoiceAgentRAG,
    private val predictiveCache: PredictiveCache,
    private val knowledgeGraph: TemporalKnowledgeGraph
) {
    // 고객 발화가 들어올 때마다 백그라운드에서 실행
    suspend fun onTranscriptUpdate(transcript: StreamingTranscript) {
        coroutineScope {
            // 1. 맥락 분석: 현재 대화 주제, 감정, 의도 파악
            val context = contextAnalyzer.analyze(transcript)

            // 2. 후속 질문 예측 (Claude Haiku 경량 호출)
            val predictions = async {
                llmClient.predictNextNeeds(
                    context = context,
                    maxPredictions = 3,
                    model = "claude-haiku-4.5"
                )
            ...
            // 3. 예측된 정보를 RAG로 미리 검색
            val prefetchResults = async {
                predictions.await().map { prediction ->
                    ragEngine.search(prediction.query, topK = 3)
                }
            }

            // 4. 결과를 초고속 캐시에 적재 (TTL: 5분)
            prefetchResults.await().flatten().forEach { result ->
                predictiveCache.put(
                    key = result.contextKey,
                    value = result,
                    ttlMs = 300_000  // 5분
                )
            }
            ...
            // 5. 지식그래프 실시간 업데이트
            knowledgeGraph.addEpisode(transcript, context)
        }
    }
}
```

### 23.3 Fast Talker (포그라운드 에이전트)

**역할**: 개입 타이밍이 감지되면 캐시에서 0.35ms 이내 즉시 조회 → 1~3단어 힌트 생성 → TTS 전달

```kotlin
class FastTalkerAgent(
    private val predictiveCache: PredictiveCache,
    private val hintGenerator: HintGenerator,
    private val ttsStreamer: CartesiaSonicStreamer
) {
    // 트리거 발동 시 즉시 실행
    suspend fun onTriggerDetected(trigger: TriggerEvent): WhisperResult {
        // 1. 캐시에서 즉시 조회 (< 0.35ms, 인메모리)
        val cachedContext = predictiveCache.get(trigger.contextKey)

        // 2. 힌트 생성 (1~3단어, LLM 없이 템플릿 or 캐시)
        val hint = if (cachedContext != null) {
            hintGenerator.fromCache(cachedContext, trigger)  // < 1ms
        } else {
            // 캐시 미스 → LLM 경량 호출 (Haiku, max_tokens=10)
            hintGenerator.fromLLM(trigger, maxTokens = 10)   // < 200ms
        }

        // 3. TTS 즉시 스트리밍 (Cartesia Sonic, TTFA < 40ms)
        ttsStreamer.streamHint(hint)

        return WhisperResult(
            text = hint.text,
            source = if (cachedContext != null) "CACHE" else "LLM",
            latencyMs = measureTimeMillis { /* 전체 */ }
        )
    }
}
```

### 23.4 PredictiveCache 구현

```kotlin
class PredictiveCache(
    private val maxSize: Int = 200,
    private val defaultTtlMs: Long = 300_000
) {
    // ConcurrentHashMap 기반 인메모리 캐시 (Android 최적화)
    ...
    fun put(key: String, value: RAGResult, ttlMs: Long = defaultTtlMs) {
        // LRU 방식으로 최대 크기 유지
...
### 23.5 레이턴시 버짓 (v2.2 듀얼 에이전트)

| 구간 | 캐시 히트 시 | 캐시 미스 시 | 비고 |
|------|------------|------------|------|
| VAD → STT 전달 | < 1ms | < 1ms | Silero VAD |
| STT 처리 | < 200ms | < 200ms | Deepgram Nova-3 스트리밍 |
| 캐시 조회 | **< 0.35ms** | - | ConcurrentHashMap |
| LLM 힌트 생성 | - | < 200ms | Claude Haiku, max_tokens=10 |
| TTS TTFA | < 40ms | < 40ms | Cartesia Sonic 3 |
| **E2E 합계** | **< 250ms** | **< 450ms** | Phase 4 목표 달성 |

---

## 24. 초저지연 스트리밍 TTS 파이프라이닝

> **v2.2 신규 추가 섹션** — Cartesia Sonic SSM 기반 TTS + 의미단위 청크 디코딩

### 24.1 Cartesia Sonic 3 통합

**왜 Cartesia Sonic인가?**
- SSM(State Space Model) 기반으로 Transformer 대비 10x 빠른 추론
- TTFA(Time-to-First-Audio): **40ms** (ElevenLabs: ~400ms)
- 스트리밍 네이티브: 텍스트가 들어오는 즉시 음성 합성 시작
- 40+ 언어 지원, 자연스러운 감정 표현

```kotlin
class CartesiaSonicStreamer(
    private val apiKey: String,
    private val voiceId: String = "ko-earbrief-whisper"
) {
    ...
    // 의미단위 청크 스트리밍
    suspend fun streamHint(hint: Hint) {
        val chunks = hint.toMeaningChunks()  // "3시에 | 비어있어요"

        chunks.forEachIndexed { index, chunk ->
            val request = CartesiaRequest(
                modelId = "sonic-3-turbo",
                transcript = chunk.text,
                voice = CartesiaVoice(id = voiceId),
                outputFormat = OutputFormat(
                    container = "raw",
                    sampleRate = 24000,
                    encoding = "pcm_s16le"
                ),
                // 핵심: 이전 청크의 상태를 이어받아 연속성 유지
                contextId = hint.contextId,
                continue_ = index > 0
            )

            // SSE 스트리밍으로 PCM 청크 수신 → 즉시 재생
            val audioFlow = cartesiaApi.streamTTS(request)
            audioFlow.collect { pcmChunk ->
                audioPlayer.enqueue(pcmChunk)  // 실시간 재생 큐
            }
        }
    }
}
...
### 24.2 LLM→TTS 파이프라이닝 (동시성 렌더링)

```
기존 (v2.1, 직렬):
  LLM 전체 응답 생성 (200ms) → TTS 전체 합성 (400ms) → 재생
  총: 600ms+

v2.2 (파이프라인):
  LLM 토큰 생성 ──→ 의미단위 버퍼 ──→ Cartesia 스트리밍 ──→ 즉시 재생
       ↓ (5ms)        ↓ (50ms)         ↓ (40ms TTFA)
  "3시에"           버퍼 flush        PCM 출력 시작
       ↓ (15ms)       ↓ (20ms)
  "비어있어요"       버퍼 flush → 이전 컨텍스트 이어서 합성
  총: ~95ms (첫 음성까지)
```

```kotlin
class StreamingPipeline(
    private val llmClient: ClaudeClient,
    private val meaningChunker: MeaningChunker,
    private val ttsStreamer: CartesiaSonicStreamer
) {
    suspend fun generateAndSpeak(trigger: TriggerEvent, context: CachedContext) {
        val contextId = UUID.randomUUID().toString()

        llmClient.streamCompletions(
            prompt = buildHintPrompt(trigger, context),
            maxTokens = 15  // 1~3단어 힌트
        ).transform { tokenDelta ->
            meaningChunker.feed(tokenDelta)
            meaningChunker.flush()?.let { chunk -> emit(chunk) }
        }.collectIndexed { index, meaningChunk ->
            // LLM 토큰이 의미단위로 모이면 즉시 TTS로 전달
            ttsStreamer.streamChunk(
                text = meaningChunk,
                contextId = contextId,
...
### 24.3 MeaningChunker (의미단위 분할기)

```kotlin
class MeaningChunker {
    private val buffer = StringBuilder()
    private val koreanParticles = setOf("은", "는", "이", "가", "을", "를", "에", "에서", "으로")

    fun feed(token: String) { buffer.append(token) }

    fun flush(): String? {
        val text = buffer.toString()
        // 한국어 조사/어미 뒤에서 분할 (의미 단위)
        val breakPoint = findNaturalBreak(text)
        if (breakPoint > 0) {
            val chunk = text.substring(0, breakPoint)
            buffer.delete(0, breakPoint)
            return chunk
        }
        // 3단어 이상 쌓이면 강제 flush
...
## 25. 시간인식 지식그래프 (Temporal Knowledge Graph)

> **v2.2 신규 추가 섹션** — Zep Graphiti 기반 시간축 포함 관계 그래프

### 25.1 왜 벡터 DB만으로는 부족한가

| 문제 | 벡터 DB (Pinecone) | 시간인식 지식그래프 (Graphiti) |
|------|-------------------|-------------------------------|
| "김 대리가 지난주에 말한 가격" | 시간 필터 수동 구현 필요 | 시간축 자동 추론 ✅ |
| "A 고객이 3번째 방문한 건" | 관계 추적 불가 | 엔티티 관계 ↔ 에피소드 자동 연결 ✅ |
| 교차 판매 추천 | 유사도 검색만 가능 | 구매 패턴 → 관련 상품 그래프 순회 ✅ |
| 정보 만료 처리 | 수동 업데이트 필요 | bi-temporal 모델로 자동 무효화 ✅ |

### 25.2 Graphiti 3-Layer 아키텍처

```
Layer 1: 에피소드 서브그래프 (Episodic)
  └─ 원본 대화 데이터 저장 (세션별)
  └─ 시간순 연결 (happens_before 관계)

Layer 2: 시맨틱 엔티티 서브그래프 (Semantic Entity)
  └─ 추출된 엔티티 간 관계 매핑
  └─ 예: [김대리] --(works_at)-→ [A기업]
         [A기업] --(purchased)-→ [제품B] --(valid_from: 2026-01)

Layer 3: 커뮤니티 서브그래프 (Community)
  └─ 고수준 요약 및 클러스터
  └─ 예: "김대리는 제품B의 주요 구매 담당자, 가격 민감형"
```

### 25.3 Kotlin 통합 (Zep Cloud API)

```kotlin
class TemporalKnowledgeGraph(
    private val zepClient: ZepClient,
    private val graphId: String
) {
    // 대화 에피소드 추가 (Slow Thinker가 호출)
    suspend fun addEpisode(transcript: StreamingTranscript, context: ConversationContext) {
        zepClient.graph.episode.add(
            graphId = graphId,
            episode = EpisodeRequest(
                name = "session_${context.sessionId}_${System.currentTimeMillis()}",
                body = transcript.finalText,
                sourceDescription = "EarBrief real-time conversation",
                referenceTime = Instant.now().toString()
            )
        )
    }

    // 시간 인식 검색 (Fast Talker가 크로스셀 추천 시 사용)
    suspend fun searchWithTime(
        query: String,
        centerDate: Instant? = null,
        maxFacts: Int = 5
    ): List<TemporalFact> {
        return zepClient.graph.search(
            graphId = graphId,
            query = query,
            centerDate = centerDate?.toString(),
            limit = maxFacts
        ).map { fact ->
            TemporalFact(
                content = fact.fact,
                validFrom = fact.validAt,
                validTo = fact.invalidAt,
                confidence = fact.score,
                sourceEntities = fact.entities
            )
        }
    }

    // 개인화된 크로스셀 추천
    suspend fun getCrossSellSuggestions(customerId: String): List<ProductSuggestion> {
        val customerFacts = searchWithTime("$customerId 구매 이력 관심 제품")
        val recentPurchases = customerFacts.filter { it.isRecent(days = 90) }
        // 그래프 순회: 구매 → 관련 제품 → 미구매 제품 필터
        return zepClient.graph.traverse(
            startEntity = customerId,
            relationship = "purchased",
            hops = 2
        ).filterNot { it.entity in recentPurchases.map { p -> p.productId } }
         .map { ProductSuggestion(it.entity, it.confidence) }
    }
}
```
...
| 정책 | 구현 |
|------|------|
| 에피소드 원본 저장 금지 | `addEpisode` 시 LLM으로 엔티티만 추출 후 원본 삭제 |
| 10분 TTL | 에피소드 데이터는 10분 후 자동 삭제, 엔티티 관계만 유지 |
| 사용자 삭제 요청 | `zepClient.graph.delete(graphId)` 즉시 전체 그래프 삭제 |
| AES-256 전송 | Zep Cloud API 전구간 TLS 1.3 암호화 |

---

## 26. LLM 추론 최적화 (Speculative Decoding & Eager EOT)

> **v2.2 신규 추가 섹션** — 응답 시작 시간을 200ms 이상 단축하는 두 가지 핵심 기법

### 26.1 Speculative Decoding (투기적 디코딩)

**원리**: 작은 draft 모델이 여러 토큰을 빠르게 예측 → 큰 target 모델이 한 번에 검증

```
기존 디코딩:
  Token1 (50ms) → Token2 (50ms) → Token3 (50ms) → ...
  TTFT: 50ms, 3토큰 완성: 150ms

Speculative Decoding:
  Draft(3 tokens, 15ms) → Target(검증 1회, 50ms, 3토큰 동시 확정)
  TTFT: 15ms, 3토큰 완성: 65ms  → 56% 단축!
```

**EarBrief 적용**: Claude Haiku의 응답이 1~3단어이므로, speculative decoding의 효과가 극대화

```kotlin
class SpeculativeLLMClient(
    private val draftModel: String = "claude-haiku-4.5",  // 소형 모델
    private val maxSpeculativeTokens: Int = 5
) {
    suspend fun generateHint(prompt: String): Flow<String> = flow {
        // Anthropic API의 speculative decoding은 서버 측에서 자동 처리
        // 클라이언트는 streaming 응답만 빠르게 수신
        val response = anthropicClient.messages.stream(
            model = draftModel,
            maxTokens = 10,
            messages = listOf(Message(role = "user", content = prompt)),
            // 스트리밍 모드에서 토큰별 즉시 전달
...
### 26.2 Eager End-of-Turn (조기 턴 종료 감지)

**원리**: 화자의 발화가 끝나기 전에 높은 확률로 턴 종료를 예측 → LLM 처리 조기 시작

```
기존 (v2.1):
  화자 발화 ─────────────────── 침묵 800ms ─── STT 확정 ─── LLM 시작
                                                                ↓ (200ms 낭비)

v2.2 Eager EOT:
  화자 발화 ──────── EagerEOT(중간 신뢰도) ─── LLM 추론 시작
                          ↓                          ↓
                    확정 EOT ──────────────── LLM 응답 거의 완료!
                                              또는 TurnResumed → 취소
```
...
```kotlin
class EagerEndOfTurnDetector(
    private val eagerThreshold: Float = 0.6f,   // 중간 신뢰도
    private val finalThreshold: Float = 0.9f,    // 확정 신뢰도
    private val eagerSilenceMs: Long = 300,      // 300ms 침묵 시 Eager
    private val finalSilenceMs: Long = 800       // 800ms 침묵 시 확정
) {
    sealed class EOTEvent {
        data class EagerEndOfTurn(val transcript: String, val confidence: Float) : EOTEvent()
        data class FinalEndOfTurn(val transcript: String) : EOTEvent()
        object TurnResumed : EOTEvent()  // 화자가 다시 말하기 시작
    }

    fun detectEOT(vadEvents: Flow<VADEvent>, sttStream: Flow<STTResult>): Flow<EOTEvent> = flow {
        var silenceStartMs: Long? = null
        var lastTranscript = ""
        var eagerFired = false

        merge(vadEvents, sttStream).collect { event ->
            when (event) {
                ...
                is VADEvent.SpeechStart -> {
                    if (eagerFired) {
                        emit(EOTEvent.TurnResumed)  // Eager 취소!
                        eagerFired = false
                    }
                    silenceStartMs = null
                }
            ...
            // 침묵 시간 체크
            silenceStartMs?.let { start ->
                val elapsed = System.currentTimeMillis() - start
                if (elapsed >= eagerSilenceMs && !eagerFired) {
                    emit(EOTEvent.EagerEndOfTurn(lastTranscript, eagerThreshold))
                    eagerFired = true
                }
                if (elapsed >= finalSilenceMs) {
                    emit(EOTEvent.FinalEndOfTurn(lastTranscript))
                    silenceStartMs = null
                    eagerFired = false
                }
            }
...
### 26.3 NVIDIA Parakeet TDT (온디바이스 STT 대안)

> **v2.2 Phase 3**: Whisper 대신 Parakeet TDT 0.6B 적용 검토

| 항목 | Whisper tiny/base | Parakeet TDT 0.6B |
|------|-------------------|-------------------|
| 파라미터 | 39M / 74M | 600M |
| WER (영어) | ~8% / ~5% | **업계 최저** |
| 추론 속도 | 1x / 0.7x | **RTFx > 2,000** |
| 스트리밍 | 불가 (배치) | **가능 (TDT 아키텍처)** |
| 한국어 | 지원 | v3 (25개 유럽어), 한국어 TBD |
| 모델 크기 | ~150MB / ~300MB | ~650MB |
| 온디바이스 | ✅ ONNX | ✅ NeMo/ONNX |

**결론**: Phase 3에서 Parakeet TDT의 한국어 지원이 확인되면 Whisper 대체, 아닐 경우 Faster-Whisper 유지

### 26.4 통합 레이턴시 비교 (v2.1 vs v2.2)

| 단계 | v2.1 정식(목표) | v2.2 Phase 4(목표) | 개선 기법 |
|------|---------------|-------------------|-----------|
| VAD | < 1ms | < 1ms | 동일 (Silero) |
| STT | < 300ms | < 200ms | Deepgram Nova-3 최적화 |
| 턴 감지 | 800ms 대기 | **300ms (Eager)** | Eager EOT |
| 캐시/RAG | < 200ms | **< 0.35ms (캐시히트)** | PredictiveCache |
| LLM | < 200ms | **< 100ms** | Speculative + 1~3단어 |
| TTS | < 400ms | **< 40ms (TTFA)** | Cartesia Sonic 3 |
| **E2E 합계** | **< 1,000ms** | **< 350ms** | **65% 단축** |

---

> **귀띔(EarBrief) 개발 기획서 v2.2** (초저지연 듀얼 에이전트 아키텍처 + 최첨단 기술 통합판)
> 본 문서는 귀띔_EarBrief_서비스기획서_v1.0 / 개발기획서_v2.1을 기반으로 작성된 통합 개발 기획서입니다.
